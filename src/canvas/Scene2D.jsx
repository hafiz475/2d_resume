...
export default function Scene2D() {
  ...
  useEffect(() => {
    ...
    // auto advance slow for cinematic scenes
    if (["sunset","sunrise","raining","storm","rainbow","winter"].includes(SCENES[index])) {
      tlRef.current = gsap.delayedCall(5.5, nextScene);
    } else {
      // short auto advance for quick scenes
      tlRef.current = gsap.delayedCall(6.5, nextScene);
    }
  ...
  return (
    <div className="scene-container" onClick={nextScene}>
